...
def mse(original, new):
    return (1 / len(original)) * sum(
        [euclid_squared(original[i], new[i]) ** 2 for i in range(len(original))]
    )
...
def main():
    if len(argv) == 4:
        with open(argv[1], "rb") as f, open(argv[2], "wb") as output:
            ...
            original_bitmap = parse_bitmap(tga[18:18+(3*height*width)], width, height)

            codebook = generate_codebook(original_bitmap, 2 ** int(argv[3]))
